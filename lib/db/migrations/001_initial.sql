-- Your Set — initial schema (set-centric, optional sessions)
-- Apply via lib/db/migrate.ts (Phase 2+)
...
CREATE INDEX idx_sets_workout ON sets(workout_id, performed_at DESC);

-- Set-first (primary history / compare / filters)
CREATE INDEX idx_sets_variant_performed ON sets(exercise_variant_id, performed_at DESC);
CREATE INDEX idx_sets_performed ON sets(performed_at DESC);
